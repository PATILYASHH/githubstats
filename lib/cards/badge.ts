...
// Brand glyphs (16x16) for social badges — replaces shields.io's simple-icons.
...
export interface BadgeOpts {
  ...
  logo?: string; // brand key, optional
  logoColor?: string; // hex without '#'
}

// A flat "for-the-badge"-style pill: optional brand logo + uppercase label.
export function renderBadge(opts: BadgeOpts): string {
  const text = opts.message.toUpperCase();
  const fontSize = 11;
  const h = 28;
  const padX = 12;
  const gap = 7;
  const hasLogo = Boolean(opts.logo && BRAND_ICONS[opts.logo]);
  const logoSize = 14;
  // for-the-badge uses generous letter-spacing.
  const spacing = 1.2;
  const tw = textWidth(text, fontSize, true) + text.length * spacing;
  const logoW = hasLogo ? logoSize + gap : 0;
  const w = Math.ceil(padX * 2 + logoW + tw);
  const color = opts.color.replace(/^#/, "");
  const logoColor = (opts.logoColor ?? "ffffff").replace(/^#/, "");

  const logoSvg = hasLogo
    ? `<svg x="${padX}" y="${(h - logoSize) / 2}" width="${logoSize}" height="${logoSize}" viewBox="0 0 16 16" fill="#${logoColor}"><path d="${BRAND_ICONS[opts.logo!]}"/></svg>`
    : "";
  const textX = padX + logoW;
  ...
  ${logoSvg}
  <text x="${textX}" y="${h / 2 + 4}" font-family="${FONT}" font-size="${fontSize}" font-weight="700" letter-spacing="${spacing}" fill="#ffffff">${escapeXml(
    text
  )}</text>
</svg>`;
}
